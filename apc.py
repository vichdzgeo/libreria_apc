...
#Libreria para el procesamiento de datos geograficos en Python (ver. 2.7) y Qgis (2.18)
#Autor: Fidel Serrano,Victor Hernandez
#Objetivo: Facilitar la creacion de scripts en los procesos de información geográfica
...
def v_interseccion(vector_a,vector_b,path_s):
    ...
    Parametros:
        ...
        vector_b =
    '''
    processing.runalg("saga:intersect",vector_a,vector_b,0,path_s)

def area_km2(path_vector,nombre_campo):
    """
    Esta funcion genera un campo nuevo y calcula el area de cada poligono en kilometros cuadrados
    Parametros:
        path_vector = ruta de la capa vectorial, debe estar en una proyeccion cartografica
                      que exrprese sun unidades lineales, ej: UTM14N o CCL
    ...
    if len(nombre_campo) >10:
        ...
        vector.dataProvider().addAttributes([QgsField(nombre_campo.lower(),QVariant.Double)])
...
def area_ha(path_vector,nombre_campo):
    """
    Esta funcion genera un campo nuevo y calcula el area de cada poligono en hectareas
    Parametros:
        path_vector = ruta de la capa vectorial, debe estar en una proyeccion cartografica
                      que exrprese sun unidades lineales, ej: UTM14N o CCL
...
def colonia_ageb(path_salida,path_agebs,path_colonias,ageb_id,col_id,campos_resampling,nombre_temp):
    ...
    QgsOverlayAnalyzer().intersection(new_geom, old_geom, join(path_salida,nombre_temp))
    intersection = QgsVectorLayer(join(path_salida,nombre_temp), "intersection", "ogr")
    ...
    for poligono in new_geom.getFeatures():
        ...
        for resamplingField in campos_resampling:
            ...
            request = QgsFeatureRequest().setFilterExpression('"'+new_geom_id+'" = '+str(poligono[new_geom_id]))
...
def est_raster_media(path_vector,path_raster,nombre_campo):
    ...
    zoneStat = QgsZonalStatistics(vector,path_raster,nombre_campo,1,QgsZonalStatistics.Mean)
...
def wgs84_a_utm(path_gws84,path_utm,zona_norte):
    ...
    if zona_norte==11:
        ...
        proyecta = QgsVectorFileWriter.writeAsVectorFormat(vlayer,path_utm16n,'utf-8',crs,"ESRI Shapefile")
    elif zona_norte==12:
...
def proy_a_wgs84(path_capa,path_wgs84):
    ...
    proyecta = QgsVectorFileWriter.writeAsVectorFormat(vlayer,path_wgs84,'utf-8',crs,"ESRI Shapefile")
...
def lista_archivos(lista_a,path,path_d,folders_check,tipo_ext):
    ...
    '''
    Esta funcion recursiva regresa una lista que contiene los archivos contenidos
    en un directorio segun el tipo de extension declarado.
    ...
    for f in os.listdir(path_d):
        ...
        if os.path.isdir(d) and d not in folders_check:
            lista_archivos(lista_a,path,d+"/",folders_check,tipo_ext)
        else:
            if os.path.isfile(d):
    ...
    for arch_shape in lista_shp:
        ...
        for prj in lista_prj:
            if capa==prj.split(".")[0]:
                coinc+=1
        if coinc==0:
            sin_proj.append("Warning: no existe el archivo prj para la capa "+arch_shape)
    return sin_proj
...
def extension_capa(path_capa):
    ...
    if vlayer.crs().authid()=="EPSG:4326":
        ...
        ruta_temp="/".join(path_capa.split("/")[:-1])+"/tp_wgs84_"+nombre+".shp"
